defmodule RetrievalNode.Ingest.Workers.DriveSync do
  ...
  An exported Doc whose content is binary (`Chunking.binary_content?/1`) also
  stages a deletion entry instead of a content row — `PendingChunks.
  insert_raw_all/1`'s own binary guard would otherwise silently drop it,
  leaving any previously-indexed chunks for that doc stuck in the index
  forever.
  ...
  defp raw_row(source, doc) do
    case Drive.export_doc(doc.doc_id) do
      {:ok, text} ->
        if Chunking.binary_content?(text) do
          # Present but unstageable as text — reconcile away whatever chunks
          # this doc had under its previous (text) content instead of
          # letting insert_raw_all/1's own guard silently drop the row.
          {:ok, deletion_row(source, doc.doc_id)}
        else
          {:ok,
           %{
             source: "drive",
