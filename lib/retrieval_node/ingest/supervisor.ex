defmodule RetrievalNode.Ingest.Supervisor do
  ...
  A second VM against the same database — a bare `iex -S mix`, any admin task
  — must never become a second writer for a source another node already
  owns. That's exactly the multi-writer bug class this whole redesign exists
  to eliminate (see the plan's "Deployment premise: one BEAM node"). Those
  processes still boot the full OTP application (`Oban.insert/1` needs a
  running Oban to resolve its config), but they configure Oban with
  `queues: []` — this module's gate reads that back:
  `:ingest_resume_on_boot` true AND `Application.get_env(:retrieval_node, Oban)`
