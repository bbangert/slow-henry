defmodule RetrievalNode.Reranking.Warmer do
  @moduledoc """
  Fires `RetrievalNode.Reranking.Serving.warmup/0` after boot without blocking
  it, retrying on failure with a bounded exponential backoff.

  Lives under `RetrievalNode.Reranking.Supervisor`, a `:rest_for_one`
    ...
    case state.warmup_fun.() do
      ...
      :error ->
        if attempt >= state.max_attempts do
          Logger.error(
            "Reranking warmup failed after #{attempt}/#{state.max_attempts} attempts; " <>
          ...
          Logger.warning(
            "Reranking warmup failed (attempt #{attempt}/#{state.max_attempts}); " <>
