defmodule RetrievalNode.Chunking.FakeImpl do
  ...
  `:fake_chunk_result` application env. Lets `ChunkFiles`/`Ingest.FileIngest`-
  scoped tests force the fallback/skip branches (`:unsupported_language`,
  `:too_large`, `:binary_content`, a crash reason) that the real
  `HeuristicImpl` can never produce.
  """
