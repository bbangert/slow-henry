defmodule RetrievalNode.Ingest.FileIngestTest do
  # async: false — mutates the :chunking_impl/:fake_chunk_with_graph_result
  # application env, and shares the SQL sandbox with the manual-mode Oban
  # instance the application tree starts.
  ...
  test "happy path: chunks + file_hash + embeddings + graph rows + raw row deleted", %{
    source: source
  } do
    ...
    force_chunk_with_graph(
      chunk_result(
        ...
        [%{name: "a", kind: :call, from: "b", line: 4}]
      )
    )
    ...
    assert summary.graph.entities == 2
    # 2 definition mentions (a, b) + 1 call mention (b calls a)
    assert summary.graph.mentions == 3
    ...
    assert Repo.aggregate(EntityMention, :count, :id) == 3
  ...
  test "scrubs secrets before chunking and records an audit row for the redacted finding", %{
    ...
    # through chunking — chunk_with_graph/2 has no graph callback here, so the
    # dispatcher wraps chunk/2's result with empty entities/references.
    prev = Application.get_env(:retrieval_node, :chunking_impl)
  ...
  test "heuristic fallback on {:error, :unsupported_language}: chunks written, no graph", %{
    source: source
  } do
    natural_key = "repo:acme/app:app.rb"
    force_chunk_with_graph({:error, :unsupported_language})
  ...
  describe "parse crash" do
    test "{:error, :boom} with default on_parse_crash: :error → {:error, :boom}, nothing written",
      ...
      force_chunk_with_graph({:error, :boom})
  ...
  describe "unindexable content" do
    test "scrub cancel (content too large) reaps the raw row AND removes the file's prior chunks",
      ...
      force_chunk_with_graph(chunk_result([{"chunk a", "a", 1, 1}]))
    ...
    test "chunker :too_large is unindexable the same way", %{source: source} do
      ...
      force_chunk_with_graph({:error, :too_large})
    ...
    test "chunker :binary_content is unindexable the same way", %{source: source} do
      ...
      force_chunk_with_graph({:error, :binary_content})
  ...
  test "whitespace-only content reconciles all old chunks away", %{source: source} do
    ...
    force_chunk_with_graph(chunk_result([{"chunk a", "a", 1, 1}, {"chunk b", "b", 2, 2}]))
  ...
  test "rollback on graph failure: no chunk rows written, raw row survives", %{source: source} do
    natural_key = "repo:acme/app:badgraph.py"

    # An unresolvable reference kind makes Graph.upsert_from_staged/3 raise
    # ArgumentError deep inside the write transaction — chunks are inserted
    # first (same transaction), so this proves the rollback actually undoes
    # them. FileIngest no longer catches this (see its moduledoc's "raises on
    # bugs" paragraph) — Ecto's rollback-then-reraise propagates straight
    # through apply/2; containing it is Ingest.SourceOwner's job now.
    force_chunk_with_graph(
      chunk_result(
        [{"def f():\n    bogus()\n", "f", 1, 2}],
        [],
        [%{name: "bogus", kind: :not_a_real_kind, from: "f", line: 1}]
      )
    )

    raw = seed_raw(source, "v1", natural_key, "badgraph.py")

    assert_raise ArgumentError, fn -> FileIngest.apply(raw, []) end
    assert chunks_for(source, "badgraph.py") == []
    assert Repo.get(PendingChunk, raw.id)
  end
end
